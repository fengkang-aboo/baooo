<?php namespace App\Http\Controllers\Upload;

use App\Http\Requests;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use Illuminate\Support\Str;
use App\Libs\UpYun;
use Validator;

class ImgController extends Controller {
	public $upyun;
	public function __construct()
	{
		$this->upyun = new UpYun(env('UPYUN_BUCKETNAME'), env('UPYUN_USERNAME'), env('UPYUN_PASSWORD'));
	}
	/**
	 * 图片上传
	 */
	public function upload(Request $request)
	{

		$data = [
			'status' => 0,
			'msg' => 'Failed!',
			'data' => null
		];
		$validator = Validator::make(
		    array('file' => $request->file('file')), 
		    array('file' => 'required|mimes:jpeg,bmp,png')
		);

		if($validator->fails()){
			$data['msg'] = "文件格式错误";
			return $data;
		}
		
		if ($file = $request->file('file')) {
			$day = date("Ymd");
			$filename = Str::random(20) .'.'. $file->getClientOriginalExtension();
			$path = $file->move(public_path()."/upload/".$day,$filename);
			$url = asset('upload').'/'.$day."/".$filename;
			$urlpath = 'upload/'.$day."/".$filename;
			$upload = new \App\Upload(['src'=>$urlpath,'originalname'=>$file->getClientOriginalName()]);
			$upload->save();
			if(\Auth::check()){
				\Auth::user()->uploads()->save($upload);
			}
			$data = [
				'status'=>1,
				'msg'=>'上传成功',
				'data'=>['url'=>$url,'path'=>$urlpath,'upload_id'=>$upload->id,'upload'=>$upload]
			];

		}
		return $data;

		// $data = [
		// 	'success' => false,
		// 	'msg' => 'Failed!',
		// 	'url' => ''
		// ];
		// if ($file = $request->file('uploadimg')) {
		// 	//$this->ratepic($_FILES['uploadimg']['tmp_name']);
		// 	$filename = Str::random(20) . '.jpg';
		// 	$this->upyun->writeFile('/' . $filename, file_get_contents($_FILES['uploadimg']['tmp_name']));

		// 	$data = [
		// 		'success'=>true,
		// 		'msg'=>'上传成功',
		// 		'url'=>'http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com/' . $filename,
		// 	];

		// }
		// return $data;
	}
	public function uploadFile(Request $request)
	{
		$data = [
			'status' => 0,
			'msg' => 'Failed!',
			'data' => null
		];
		$validator = Validator::make(
		    array('file' => $request->file('file')), 
		    array('file' => 'required')
		);

		if($validator->fails()){
			$data['msg'] = "文件格式错误";
			return $data;
		}
		
		if ($file = $request->file('file')) {
			$day = date("Ymd").'/'.Str::random(20);
			$filename = $file->getClientOriginalName();
			$path = $file->move(public_path()."/upload/".$day,$filename);
			$url = asset('upload').'/'.$day."/".$filename;
			$urlpath = 'upload/'.$day."/".$filename;
			$upload = new \App\Upload(['src'=>$urlpath,'originalname'=>$file->getClientOriginalName()]);
			$upload->save();
			if(\Auth::check()){
				\Auth::user()->uploads()->save($upload);
			}
			$data = [
				'status'=>1,
				'msg'=>'上传成功',
				'data'=>['url'=>$url,'path'=>$urlpath,'upload_id'=>$upload->id,'upload'=>$upload]
			];

		}
		return $data;

		// $data = [
		// 	'success' => false,
		// 	'msg' => 'Failed!',
		// 	'url' => ''
		// ];
		// if ($file = $request->file('uploadimg')) {
		// 	//$this->ratepic($_FILES['uploadimg']['tmp_name']);
		// 	$filename = Str::random(20) . '.jpg';
		// 	$this->upyun->writeFile('/' . $filename, file_get_contents($_FILES['uploadimg']['tmp_name']));

		// 	$data = [
		// 		'success'=>true,
		// 		'msg'=>'上传成功',
		// 		'url'=>'http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com/' . $filename,
		// 	];

		// }
		// return $data;
	}

	/**
	 * simditor编辑器上传
	 */
	public function upload0(Request $request)
	{
		$data = [
			'success' => false,
			'msg' => 'Failed!',
			'file_path' => ''
		];
		if ($file = $request->file('simditor_upload_img')) {
			//$this->ratepic($_FILES['simditor_upload_img']['tmp_name']);
			$filename = Str::random(20) . '.jpg';
			$this->upyun->writeFile('/' . $filename, file_get_contents($_FILES['simditor_upload_img']['tmp_name']));

			$data = [
				'success'=>true,
				'msg'=>'上传成功',
				'file_path'=>'http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com/' . $filename,
			];

		}
		return $data;
	}

	/**
	 * wangeditor编辑器上传
	 */
	public function upload2(Request $request)
	{
		$data = [
			'success' => false,
			'msg' => 'Failed!',
			'file_path' => ''
		];
		if ($file = $request->file('wangEditorH5File')) {
			$filename = Str::random(20) . '.jpg';
			$this->upyun->writeFile('/' . $filename, file_get_contents($_FILES['wangEditorH5File']['tmp_name']));

			$data = [
				'success'=>true,
				'msg'=>'上传成功',
				'file_path'=>'http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com/' . $filename,
			];

		}
		return $data['file_path'];
	}
	public function uploadImg($input_name,$path='',Request $request){
		$file='';
		$path=$path?$path:'/';
		if( $request->hasFile($input_name) && $request->file($input_name)->isValid() ){
			//$this->ratepic($_FILES[$input_name]['tmp_name']);
			$filename = Str::random(20).'.jpg';
			$this->upyun->writeFile($path . $filename, file_get_contents($_FILES[$input_name]['tmp_name']));
			$file = 'http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com'.$path . $filename;
		}
		return $file;
	}
	/*public function uploadFile($input_name,$path='',Request $request){
		$file='';
		$path=$path?$path.'/':'/';
		if( $request->hasFile($input_name) && $request->file($input_name)->isValid() ){
			//$this->ratepic($_FILES[$input_name]['tmp_name']);
			$filename = Str::random(20).'.'.$request->file($input_name)->getClientOriginalExtension();
			$this->upyun->writeFile($path . $filename, file_get_contents($_FILES[$input_name]['tmp_name']));
			$file = 'http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com'.$path . $filename;
		}
		return $file;
	}*/
	public function uploadVideo($input_name,$path='',Request $request){
		$file='';
		if( $request->hasFile($input_name) && $request->file($input_name)->isValid() ){
			$filename = Str::random(20).'.'.$request->file($input_name)->getClientOriginalExtension();
			$res = $request->file($input_name)->move(base_path().'/public/uploads/', $filename);
			$file = asset('/uploads/'.$filename);
		}
		return $file;
	}
	public function unlinkfile($file){
		$file =  str_replace('http://'.env('UPYUN_BUCKETNAME').'.b0.upaiyun.com','',$file);
		if(file_exists($file)){
			@unlink($file);
		}
	}
	private function ratepic($tmp_name=''){
		$exif_info = exif_read_data($tmp_name);
		if(!empty($exif_info['Orientation'])) {
			$source = imagecreatefromjpeg($tmp_name);
			$image='';
			switch($exif_info['Orientation']) {
				case 8:
					$image = imagerotate($source,90,0);
					break;
				case 3:
					$image = imagerotate($source,180,0);
					break;
				case 6:
					$image = imagerotate($source,-90,0);
					break;
			}
			if($image) {
				imagejpeg($image, $tmp_name);
				imagedestroy($source);
			}
		}
	}
}
